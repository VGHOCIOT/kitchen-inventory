"""
Ingredient substitution seed data.

Defines common cooking substitutions with conversion ratios and quality scores.
Seeded via the /api/v1/recipes/seed-substitutions endpoint.

Format:
    original: canonical ingredient name being replaced
    substitute: canonical ingredient name to use instead
    ratio: quantity multiplier (1.0 = same amount, 0.75 = use 25% less)
    quality_score: 1-5 (3=decent swap, 4=good, 5=excellent drop-in)
    bidirectional: if True, seeder creates both directions (reverse ratio auto-calculated)

Names must match canonical ingredient names from ingredient_aliases.py or fresh_weights.py.
"""

INGREDIENT_SUBSTITUTION_SEEDS: list[dict] = [
    # ── Dairy & Fats ─────────────────────────────────────────────────────────
    {
        "original": "butter",
        "substitute": "margarine",
        "ratio": 1.0,
        "quality_score": 4,
        "notes": "Good for most cooking; less ideal for flaky pastry",
        "bidirectional": True,
    },
    {
        "original": "butter",
        "substitute": "coconut oil",
        "ratio": 0.8,
        "quality_score": 3,
        "notes": "Use less coconut oil; adds slight coconut flavor",
        "bidirectional": True,
    },
    {
        "original": "butter",
        "substitute": "olive oil",
        "ratio": 0.75,
        "quality_score": 2,
        "notes": "Works for sautéing; not suitable for baking",
        "bidirectional": False,
    },
    {
        "original": "sour cream",
        "substitute": "greek yogurt",
        "ratio": 1.0,
        "quality_score": 5,
        "notes": "Excellent swap; similar tang and consistency",
        "bidirectional": True,
    },
    {
        "original": "heavy cream",
        "substitute": "coconut cream",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Works for curries and soups; adds coconut flavor",
        "bidirectional": True,
    },
    {
        "original": "cream cheese",
        "substitute": "ricotta",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Lighter texture; works in dips and fillings",
        "bidirectional": True,
    },

    # ── Cooking Oils ─────────────────────────────────────────────────────────
    {
        "original": "olive oil",
        "substitute": "vegetable oil",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Neutral flavor; good for high-heat cooking",
        "bidirectional": True,
    },
    {
        "original": "vegetable oil",
        "substitute": "canola oil",
        "ratio": 1.0,
        "quality_score": 5,
        "notes": "Near-identical for cooking purposes",
        "bidirectional": True,
    },
    {
        "original": "sesame oil",
        "substitute": "olive oil",
        "ratio": 1.0,
        "quality_score": 1,
        "notes": "Loses sesame flavor; works as a base oil substitute only",
        "bidirectional": False,
    },

    # ── Sweeteners ───────────────────────────────────────────────────────────
    {
        "original": "sugar",
        "substitute": "honey",
        "ratio": 0.75,
        "quality_score": 3,
        "notes": "Use less honey; reduce other liquids slightly in baking",
        "bidirectional": False,
    },
    {
        "original": "sugar",
        "substitute": "maple syrup",
        "ratio": 0.75,
        "quality_score": 3,
        "notes": "Use less syrup; reduce other liquids slightly in baking",
        "bidirectional": False,
    },
    {
        "original": "honey",
        "substitute": "maple syrup",
        "ratio": 1.0,
        "quality_score": 4,
        "notes": "Similar sweetness and consistency",
        "bidirectional": True,
    },
    {
        "original": "brown sugar",
        "substitute": "sugar",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Loses molasses depth; add a splash of molasses if available",
        "bidirectional": False,
    },

    # ── Herbs ────────────────────────────────────────────────────────────────
    {
        "original": "parsley",
        "substitute": "cilantro",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Different flavor profile; works as garnish",
        "bidirectional": True,
    },
    {
        "original": "thyme",
        "substitute": "oregano",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Similar earthy profile; common in Mediterranean cooking",
        "bidirectional": True,
    },
    {
        "original": "basil",
        "substitute": "oregano",
        "ratio": 0.75,
        "quality_score": 1,
        "notes": "Oregano is stronger; use less",
        "bidirectional": False,
    },
    {
        "original": "dill",
        "substitute": "fennel",
        "ratio": 0.5,
        "quality_score": 1,
        "notes": "Similar anise notes; fennel is stronger",
        "bidirectional": False,
    },
    {
        "original": "rosemary",
        "substitute": "thyme",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Both are woody herbs; works in roasts and stews",
        "bidirectional": True,
    },
    {
        "original": "mint",
        "substitute": "basil",
        "ratio": 1.0,
        "quality_score": 1,
        "notes": "Works in some salads and Thai dishes; different flavor",
        "bidirectional": False,
    },

    # ── Alliums ──────────────────────────────────────────────────────────────
    {
        "original": "onion",
        "substitute": "shallot",
        "ratio": 0.5,
        "quality_score": 4,
        "notes": "Shallots are more concentrated; use less by weight",
        "bidirectional": False,
    },
    {
        "original": "shallot",
        "substitute": "onion",
        "ratio": 2.0,
        "quality_score": 3,
        "notes": "Use more onion; milder flavor",
        "bidirectional": False,
    },
    {
        "original": "green onion",
        "substitute": "chives",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Similar mild onion flavor; best as garnish",
        "bidirectional": True,
    },
    {
        "original": "red onion",
        "substitute": "onion",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Loses color; slightly different flavor when raw",
        "bidirectional": True,
    },
    {
        "original": "leek",
        "substitute": "onion",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Onion is sharper; works in cooked dishes",
        "bidirectional": False,
    },

    # ── Acids / Citrus ───────────────────────────────────────────────────────
    {
        "original": "lemon",
        "substitute": "lime",
        "ratio": 1.0,
        "quality_score": 5,
        "notes": "Nearly interchangeable in most recipes",
        "bidirectional": True,
    },
    {
        "original": "white vinegar",
        "substitute": "apple cider vinegar",
        "ratio": 1.0,
        "quality_score": 4,
        "notes": "Slightly fruity; works in dressings and marinades",
        "bidirectional": True,
    },
    {
        "original": "rice vinegar",
        "substitute": "apple cider vinegar",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "ACV is slightly stronger; works in Asian-style dressings",
        "bidirectional": False,
    },

    # ── Proteins ─────────────────────────────────────────────────────────────
    {
        "original": "chicken breast",
        "substitute": "chicken thigh",
        "ratio": 1.0,
        "quality_score": 4,
        "notes": "Thigh is fattier and more forgiving; great swap",
        "bidirectional": True,
    },
    {
        "original": "ground beef",
        "substitute": "ground turkey",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Leaner; works in tacos, pasta sauce, meatballs",
        "bidirectional": True,
    },
    {
        "original": "ground beef",
        "substitute": "ground pork",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Fattier and slightly sweeter; great in meatballs and dumplings",
        "bidirectional": True,
    },
    {
        "original": "ground beef",
        "substitute": "ground chicken",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Much leaner; may need added fat for moisture",
        "bidirectional": False,
    },
    {
        "original": "ground turkey",
        "substitute": "ground chicken",
        "ratio": 1.0,
        "quality_score": 4,
        "notes": "Nearly identical in leanness and texture; interchangeable",
        "bidirectional": True,
    },
    {
        "original": "bacon",
        "substitute": "turkey bacon",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Leaner with less fat; crisps up similarly; halal/lower-calorie option",
        "bidirectional": True,
    },
    {
        "original": "bacon",
        "substitute": "chicken bacon",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Leaner and milder; works as a direct swap in sandwiches and wraps",
        "bidirectional": True,
    },
    {
        "original": "bacon",
        "substitute": "canadian bacon",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Much leaner back cut; lacks fat for rendering but works in sandwiches",
        "bidirectional": False,
    },
    {
        "original": "pork chop",
        "substitute": "chicken breast",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Leaner; adjust cooking time — chicken cooks faster",
        "bidirectional": False,
    },
    {
        "original": "pork tenderloin",
        "substitute": "chicken breast",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Both are lean cuts; similar cooking methods work well",
        "bidirectional": True,
    },
    {
        "original": "italian sausage",
        "substitute": "chorizo",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Chorizo is spicier and smokier; works in pasta, rice dishes",
        "bidirectional": True,
    },
    {
        "original": "italian sausage",
        "substitute": "breakfast sausage",
        "ratio": 1.0,
        "quality_score": 1,
        "notes": "Different seasoning profile; works in crumbled applications",
        "bidirectional": False,
    },
    {
        "original": "ribeye steak",
        "substitute": "sirloin steak",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Sirloin is leaner; slightly less tender but close in flavor",
        "bidirectional": True,
    },
    {
        "original": "steak",
        "substitute": "chicken breast",
        "ratio": 1.0,
        "quality_score": 1,
        "notes": "Very different texture; adjust cooking method significantly",
        "bidirectional": False,
    },
    {
        "original": "shrimp",
        "substitute": "scallop",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Different texture; works in stir-fries and pastas",
        "bidirectional": True,
    },
    {
        "original": "salmon",
        "substitute": "tuna",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Different fat content; works for steaks and grilling",
        "bidirectional": True,
    },
    {
        "original": "tofu",
        "substitute": "tempeh",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Tempeh is firmer with nuttier flavor; crumble or slice",
        "bidirectional": True,
    },

    # ── Vegetables ───────────────────────────────────────────────────────────
    {
        "original": "potato",
        "substitute": "sweet potato",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Sweeter flavor; works for roasting, mashing, soups",
        "bidirectional": True,
    },
    {
        "original": "spinach",
        "substitute": "kale",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Kale is tougher; cook longer or massage if raw",
        "bidirectional": True,
    },
    {
        "original": "broccoli",
        "substitute": "cauliflower",
        "ratio": 1.0,
        "quality_score": 4,
        "notes": "Very similar cooking properties; milder flavor",
        "bidirectional": True,
    },
    {
        "original": "zucchini",
        "substitute": "eggplant",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Different texture; works in stir-fries and grilling",
        "bidirectional": True,
    },
    {
        "original": "bell pepper",
        "substitute": "red bell pepper",
        "ratio": 1.0,
        "quality_score": 5,
        "notes": "Red is sweeter; otherwise identical",
        "bidirectional": True,
    },
    {
        "original": "bell pepper",
        "substitute": "green bell pepper",
        "ratio": 1.0,
        "quality_score": 5,
        "notes": "Green is more bitter; otherwise identical",
        "bidirectional": True,
    },
    {
        "original": "butternut squash",
        "substitute": "sweet potato",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Similar sweetness and texture when roasted or in soups",
        "bidirectional": True,
    },
    {
        "original": "mushroom",
        "substitute": "portobello mushroom",
        "ratio": 1.0,
        "quality_score": 4,
        "notes": "Larger but same family; slice to match",
        "bidirectional": True,
    },
    {
        "original": "mushroom",
        "substitute": "shiitake mushroom",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "More umami; great in Asian dishes",
        "bidirectional": True,
    },

    # ── Tomatoes ──────────────────────────────────────────────────────────────
    {
        "original": "tomato",
        "substitute": "cherry tomato",
        "ratio": 1.0,
        "quality_score": 4,
        "notes": "Sweeter; use same weight, halved or quartered",
        "bidirectional": True,
    },

    # ── Eggs ─────────────────────────────────────────────────────────────────
    {
        "original": "egg",
        "substitute": "banana",
        "ratio": 1.0,
        "quality_score": 1,
        "notes": "1 egg ≈ 1 mashed banana; for baking only, adds sweetness",
        "bidirectional": False,
    },

    # ── Starches & Grains ────────────────────────────────────────────────────
    {
        "original": "all-purpose flour",
        "substitute": "whole wheat flour",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Denser, nuttier result; may need slightly more liquid; not ideal for delicate bakes",
        "bidirectional": False,
    },
    {
        "original": "whole wheat flour",
        "substitute": "all-purpose flour",
        "ratio": 1.0,
        "quality_score": 4,
        "notes": "Lighter result; loses some fiber and nuttiness but otherwise a clean swap",
        "bidirectional": False,
    },
    {
        "original": "all-purpose flour",
        "substitute": "bread flour",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Higher gluten; slightly chewier result; works well in most recipes",
        "bidirectional": True,
    },

    # ── Dairy (plant-based swaps) ─────────────────────────────────────────────
    {
        "original": "milk",
        "substitute": "oat milk",
        "ratio": 1.0,
        "quality_score": 4,
        "notes": "Closest plant-based swap; works in baking and cooking",
        "bidirectional": True,
    },
    {
        "original": "milk",
        "substitute": "almond milk",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Thinner and slightly nutty; works in most recipes",
        "bidirectional": True,
    },
    {
        "original": "buttermilk",
        "substitute": "milk",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Add 1 tbsp vinegar or lemon juice per cup of milk; let sit 5 min",
        "bidirectional": False,
    },
    {
        "original": "sour cream",
        "substitute": "cream cheese",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Richer and less tangy; works in dips and spreads",
        "bidirectional": True,
    },

    # ── Acids ────────────────────────────────────────────────────────────────
    {
        "original": "lemon",
        "substitute": "white vinegar",
        "ratio": 0.5,
        "quality_score": 2,
        "notes": "Use half the amount; provides acidity without citrus flavor",
        "bidirectional": False,
    },

    # ── Broths ───────────────────────────────────────────────────────────────
    {
        "original": "chicken broth",
        "substitute": "vegetable broth",
        "ratio": 1.0,
        "quality_score": 4,
        "notes": "Works in most soups, stews, and sauces",
        "bidirectional": True,
    },
    {
        "original": "beef broth",
        "substitute": "chicken broth",
        "ratio": 1.0,
        "quality_score": 2,
        "notes": "Lighter flavor; works in stews and braises",
        "bidirectional": False,
    },

    # ── Concentrated / Dried / Liquid Forms ──────────────────────────────────
    # Ratios represent how much of the substitute equals the same flavor impact
    # as the original, by weight. Both ends share the same base unit (g or ml).
    {
        "original": "garlic",
        "substitute": "garlic powder",
        "ratio": 0.17,
        "quality_score": 3,
        "notes": "1 clove (~3 g) ≈ 0.5 g garlic powder (~¼ tsp); concentrated flavor",
        "bidirectional": False,
    },
    {
        "original": "garlic powder",
        "substitute": "garlic",
        "ratio": 6.0,
        "quality_score": 3,
        "notes": "Use ~6× more fresh garlic by weight; mince finely",
        "bidirectional": False,
    },
    {
        "original": "ginger",
        "substitute": "ground ginger",
        "ratio": 0.2,
        "quality_score": 3,
        "notes": "1 tbsp fresh (~6 g) ≈ 1.2 g ground ginger (~¼ tsp); concentrated",
        "bidirectional": False,
    },
    {
        "original": "ground ginger",
        "substitute": "ginger",
        "ratio": 5.0,
        "quality_score": 3,
        "notes": "Use ~5× more fresh ginger by weight; grate or mince",
        "bidirectional": False,
    },
    {
        "original": "onion",
        "substitute": "onion powder",
        "ratio": 0.1,
        "quality_score": 3,
        "notes": "1 medium onion (~110 g) ≈ 11 g onion powder (~1 tbsp); concentrated",
        "bidirectional": False,
    },
    {
        "original": "onion powder",
        "substitute": "onion",
        "ratio": 10.0,
        "quality_score": 3,
        "notes": "Use ~10× more fresh onion by weight",
        "bidirectional": False,
    },
    {
        "original": "tomato",
        "substitute": "tomato paste",
        "ratio": 0.17,
        "quality_score": 3,
        "notes": "3 tbsp fresh tomato (~50 g) ≈ 1 tbsp paste (~16 g); add water to thin if needed",
        "bidirectional": False,
    },
    {
        "original": "tomato paste",
        "substitute": "tomato",
        "ratio": 6.0,
        "quality_score": 2,
        "notes": "Use ~6× more fresh tomato by weight and reduce down; texture will differ",
        "bidirectional": False,
    },
    {
        "original": "lemon",
        "substitute": "lemon juice",
        "ratio": 0.45,
        "quality_score": 4,
        "notes": "1 medium lemon (~58 g) yields ~26 ml juice; use bottled lemon juice by weight",
        "bidirectional": False,
    },
    {
        "original": "lemon juice",
        "substitute": "lemon",
        "ratio": 2.2,
        "quality_score": 4,
        "notes": "~2.2× lemon weight gives equivalent juice; squeeze fresh",
        "bidirectional": False,
    },
    {
        "original": "lime",
        "substitute": "lime juice",
        "ratio": 0.45,
        "quality_score": 4,
        "notes": "1 lime yields ~45% of its weight as juice; use bottled lime juice",
        "bidirectional": False,
    },
    {
        "original": "lime juice",
        "substitute": "lime",
        "ratio": 2.2,
        "quality_score": 4,
        "notes": "~2.2× lime weight gives equivalent juice; squeeze fresh",
        "bidirectional": False,
    },
    {
        "original": "butter",
        "substitute": "ghee",
        "ratio": 0.8,
        "quality_score": 4,
        "notes": "Ghee is clarified butter; use slightly less — higher fat concentration",
        "bidirectional": False,
    },
    {
        "original": "ghee",
        "substitute": "butter",
        "ratio": 1.25,
        "quality_score": 4,
        "notes": "Butter has more water content; use slightly more",
        "bidirectional": False,
    },
    {
        "original": "milk",
        "substitute": "evaporated milk",
        "ratio": 0.5,
        "quality_score": 4,
        "notes": "Evaporated milk is double-concentrated; use half the amount, dilute with water",
        "bidirectional": False,
    },
    {
        "original": "evaporated milk",
        "substitute": "milk",
        "ratio": 2.0,
        "quality_score": 4,
        "notes": "Use double the milk to approximate evaporated milk concentration",
        "bidirectional": False,
    },
    {
        "original": "soy sauce",
        "substitute": "tamari",
        "ratio": 1.0,
        "quality_score": 5,
        "notes": "Near-identical; tamari is gluten-free and slightly richer",
        "bidirectional": True,
    },
    {
        "original": "soy sauce",
        "substitute": "coconut aminos",
        "ratio": 1.0,
        "quality_score": 3,
        "notes": "Slightly sweeter and less salty; works in most Asian dishes",
        "bidirectional": True,
    },
]

...
async def get_average_weight(ingredient_name: str) -> Optional[float]:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if not food_id:
                logger.debug(f"[CNF] Could not extract food_id from response for '{ingredient_name}'")
                return None
            ...
            logger.debug(f"[CNF] '{description}' has no usable unit serving sizes for '{ingredient_name}'")
            return None
